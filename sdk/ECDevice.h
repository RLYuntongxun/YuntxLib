...
/// 设置业务超时时间
/// @param ackSecond 发送到达服务器响应超时时间。 单位秒（备用，目前未启用）
/// @param respSecond 业务成功超时时间。 单位秒
- (NSInteger)setTimeOutAckResp:(NSInteger)ackSecond respSecond:(NSInteger)respSecond;

/// 设置文件发送应答超时时间, 建议使用默认值
/// @param respSecond 响应超时时间。 单位秒 -1 默认
- (NSInteger)setTimeOutFileResp:(NSInteger)respSecond;
