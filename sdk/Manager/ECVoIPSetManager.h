...
/// 控制媒体库的视频码率是用手动模式还是自动调节模式，默认是自动模式（manualMode是false),如果调用，需要在建立视频通话前
/// @param manualMode true：自动；false:手动 ,默认false
- (void)setVideoCodecManualMode:(BOOL)manualMode;
